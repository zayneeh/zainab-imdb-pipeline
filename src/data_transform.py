...
def clean_data(file_path: str) -> pl.DataFrame:
    ...
    if "releaseDate" in df.columns:
        try:
            df = df.with_columns(pl.col("releaseDate").str.to_date("%Y-%m-%d", strict=False))
        except Exception as e:
            print(f"Error parsing releaseDate: {e}")
            # If parsing fails, keep the column as a string
            df = df.with_columns(pl.col("releaseDate").cast(pl.Utf8))
